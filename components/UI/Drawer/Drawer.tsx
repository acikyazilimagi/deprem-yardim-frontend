...
const Drawer = () => {
  ...
  const list = useMemo(() => {
    ...
    const openSourceUrl = () => {
      //even if there is tweet id in extra parameters no channel returned from source
      if (extraParams?.tweet_id) {
        router.push(
          `https://twitter.com/${extraParams.user_id || "anyuser"}/status/${
            extraParams.tweet_id
          }`
        );
        return;
      }
